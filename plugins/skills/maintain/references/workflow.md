...
- **Out of context (huge backlog):** you are *already* processing oldest-first (it's a general rule — see Rules below); a big backlog only changes *where you stop*, not the order. **Cap each run: deeply process at most ~10 sessions (or ~2,000 filtered messages), whichever comes first, then STOP.** When you stop — at the cap or because context is running low — leave every remaining (newer) session's cursor **untouched** so the next tick continues from exactly there. Never flip to newest-first, and never advance a cursor for a session you didn't actually extract from. Deferring (cursor unchanged, retried next tick) is always correct; consuming-without-extracting (cursor advanced, content lost) is the failure this prevents
...
- **Never advance a cursor without real extraction.** A committed cursor means "this content is captured." If you skim or skip a session under context pressure, leave its cursor untouched so the next tick re-reads it. **Cap each run at ~10 sessions / ~2,000 filtered messages** and stop cleanly rather than racing to "finish" a backlog by marking sessions done. Silently over-advancing cursors loses content permanently — it is the worst failure mode, worse than a slow backlog. (A 4-day backlog was consumed-but-unextracted exactly this way on 2026-05-30.)
