...
- `publication`: the non-secret remote, branch, repository, credential ref,
  and credential-service binding. Informational only; the Memory service
...
Leave the commit unpublished and return. The Memory service validates the
exact commit against `repository_head` and publishes it with its own
credential. Acquiring any credential, invoking `git push`, or contacting any
remote from this run is prohibited; the `publication` binding in the envelope
is informational only. Never amend, replace, or discard the semantic commit.
